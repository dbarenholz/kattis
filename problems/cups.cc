...
int main() {
    ...
    WHILE(n) {
        ...
        try {
            ...
            // First element is string, not integer
            diameter = stoi(b) * 2;
            color = a;
        }

        cups.emplace_back(diameter, color);
    }

    // Sort cups based on int value, then print out
    sort(cups.begin(), cups.end());
    for (pair<int, string> const &cup: cups) {
        OUT(cup.second)
    }
