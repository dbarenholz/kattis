...
int main() {
    ...
    WHILE(tc) {
        str student;
        char slash;
        int start_year, birth_year, n_courses, skip;

        // Skip information from the dates by reading `char int char int`.
        cin >> student
            >> start_year >> slash >> skip >> slash >> skip
            >> birth_year >> slash >> skip >> slash >> skip
            >> n_courses;

        // This must be a separate check.... dumb question.
        if (start_year >= 2010) {
            cout << student << " " << "eligible\n";
        } else if (birth_year >= 1991) {
            cout << student << " " << "eligible\n";
        } else if (n_courses >= 41) {
            cout << student << " " << "ineligible\n";
        } else {
            cout << student << " " << "coach petitions\n";
        }
    }
    return 0;
}
