...
int main() {
    ...
    // No rooms booked - room 1 is available.
    if (n == 0) {
        OUT("1")
        return 0;
    }
    ...
    // Keep track of available rooms using a boolean array.
    // `false` means that the room is available,
    // `true` means that the room is not available.
    bool arr[r + 1]{false};

    // Read indices of the booked rooms.
    WHILE(n) {
        int i;
        cin >> i;
        arr[i] = true;
    ...
    // Loop through the rooms, and find the first available room.
    FOR(i, 1, r + 1, 1) {
        if (!arr[i]) {
            OUT(i)
            return 0;
