...
int main() {
    ...
    ll n, max_pages_per_sitting, minutes_per_page_for_me, minutes_per_page_for_meowth;
    cin >> n >> max_pages_per_sitting >> minutes_per_page_for_me >> minutes_per_page_for_meowth;

    ll time_spent_reading = 0;
    ll pages_read = 0;
    FOR(i, 1, INF, 1) {
        if (i % n == 0) {
            time_spent_reading += minutes_per_page_for_meowth;
        } else {
            if (pages_read >= max_pages_per_sitting) {
                break;
            }
            time_spent_reading += minutes_per_page_for_me;
            pages_read++;
        }
    }

    OUT(time_spent_reading);
    return 0;
}
