...
int main() {
    ...
    // can use absolute value between points
    // then take care to remove half width to get to boundary
    ld x_dist = max(abs(x - rx) - (rwidth / 2.0), (ld) 0.0);
    ld y_dist = max(abs(y - ry) - (rheight / 2.0), (ld) 0.0);
