...
int main() {
    ...
    bool smiley = regex_search(s, smiley_regex);
    bool frowny = regex_search(s, frowny_regex);

    // only smiley, no frowny -> alive
    if (smiley && !frowny) {
        OUT("alive")
        return 0;
    }
    // only frowny, no smiley -> undead
    if (frowny && !smiley) {
        OUT("undead")
        return 0;
    }
    // both smiley and frowny -> double agent
    // interesting note: clion thinks this is always true - it isn't.
    if (smiley && frowny) {
        OUT("double agent")
        return 0;
    }
